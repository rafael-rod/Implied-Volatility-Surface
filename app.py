...
st.markdown(
    ...
    **Visualize the implied volatility surface of options for an asset using the Black-Scholes model.**
    Customize the parameters in the tabs for detailed analysis.
    """
)
...
# Function to get the spot price with retries
...
with st.sidebar:
    ...
    dividend_yield = st.slider(
        ...
        help="Dividend yield (e.g., 0.013 for 1.3%)."
    )
    ...
    min_strike_pct = st.slider(
        ...
        help="Min strike price percentage of the spot price."
    )

    max_strike_pct = st.slider(
        'Max Strike Price (% of Spot Price)',
        min_value=51.0, max_value=200.0, value=120.0, step=1.0,
        help="Max strike price percentage of the spot price."
    )

    if min_strike_pct >= max_strike_pct:
        st.error('⚠️ Min percentage must be less than max percentage.')
        st.stop()
...
if not exp_dates:
    ...
    if not option_data:
        ...
        # Get the spot price with retries
        spot_price = get_spot_price(ticker_symbol)
        if spot_price is None:
            st.stop()  # Stop execution if no spot price is available
